...
photon_t photon_init(u64 init_a, u64 init_b) {
  ...
  u64 mix = init_a ^ (init_b << 1) ^ 0x9E3779B97F4A7C15ULL;
...
void generate_block(photon_t * randomp) {
  ...
  for (i = 0; i < 4; ++i) {
    randomp->b += randomp->a;
    randomp->c ^= randomp->a;
    mix[i] += stdc_rotate_left(randomp->b, 9);

    for (j = 0; j < 5; ++j) {
      randomp->elements[k] += stdc_rotate_left(mix[i], 25);
  ...
  randomp->b += stdc_rotate_left(mix[0], 23);
  randomp->b ^= mix[1];
  randomp->c += stdc_rotate_left(mix[2], 13);
  randomp->c ^= mix[3];
...
{ return photon_init(initial, initial>>32); }

u32 random32(photon_t * randomp)
{ return photon32(randomp); }
